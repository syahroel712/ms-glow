...
if ($_GET['action'] == 'input') {
	$tanggal = date('Y-m-d');
	$sekarang = date('ymd');
	$carikode = mysql_query("SELECT max(nota) from tb_kanvas");
	...
	$thn_skr = date('Y');
	$bln_skr = date('m');
	$carikode = mysql_query("SELECT * FROM tb_kanvas WHERE id_user='$kode_user' AND YEAR(tgl_beli)='$thn_skr' AND MONTH(tgl_beli)='$bln_skr' ORDER BY tgl_beli DESC");
	$datakode = mysql_fetch_row($carikode);
	...
	//hapus item tmp
	mysql_query("DELETE from kanvas_tmp where id_user='$kode_user'");
...
?>
	...
	<div class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
					<h1 class="m-0 text-dark">Kanvas</h1>
				</div><!-- /.col -->
				<div class="col-sm-6">
					<ol class="breadcrumb float-sm-right">
						<li class="breadcrumb-item"><a href="#">Home</a></li>
						<li class="breadcrumb-item active">Transaksi Kanvas</li>
					</ol>
				</div><!-- /.col -->
			</div><!-- /.row -->
		</div><!-- /.container-fluid -->
				...
				<div class="col-lg-12">
					<div class="card">
						<div class="card-header">
							<h3 class="card-title">Transaksi Kanvas ( <span class="text-bold" id="kasir"><?= $data_user['nama_lengkap']; ?></span> )</h3>
						</div>
						<div class="card-body">
							<div class="row">
								<div class="col-lg-4">
									<form class="form-horizontal">
										<div class="card-body">
											...
											<div class="form-group row">
												<label for="nm" class="col-sm-4 col-form-label">Pelanggan</label>
												<div class="col-sm-8">
													<select name="nm" id="nm" data-placeholder="Pilih Pelanggan" class="select2bs4" onchange="changeValue2(this.value)" style="width: 100%;">
														<option value=""></option>
														<?php
														$q5 = mysql_query("SELECT * FROM tb_member ORDER BY nm");
														while ($row = mysql_fetch_array($q5)) { ?>
															<option value="<?= $row['id_member'] ?>"><?= $row['nm'] ?></option>
														<?php
														}
														?>
													</select>
	...
	<script src="assets/js/suggest_kanvas.js"></script>
	<script>
		$(function() {
			$("#nonota").focus();
			$("#subtotal").val(0);
		...
		$("#simpan").click(function() {
			...
			var nm = $("#nm").val();
			var kode_user = '<?php echo $_SESSION['kode_user']; ?>';
			if (nonota == "") {
				...
				$.ajax({
					url: 'inc/proses_simpan_kanvas.php',
					data: 'nonota=' + nonota + '&tgljual=' + tgljual + '&pelanggan=' + pelanggan + '&jns=' + jns + '&kasir=' + kasir + '&subtotal=' + subtotal + '&diskonpersen=' + diskonpersen + '&diskonharga=' + diskonharga + '&totalharga=' + totalharga + '&bayar=' + bayar + '&kembalian=' + kembalian + '&ppn=' + ppn + '&nm=' + nm + '&kode_user=' + kode_user, 
					type: 'POST',
					success: function(msg) {
						$("#hasil").html(msg);
					}
				...
				for (var i = 1; i < ke; i++) {
					...
					$.ajax({
						url: 'inc/proses_simpan_barang_kanvas.php',
						type: 'post',
				...
				alert("Penjualan Kanvas Telah Tersimpan");
				window.location.href = "?page=kanvas&action=input";
				window.open("cetak_struk_kanvas.php?id="+nonota,"Faktur Penjualan","height=700,width=700,scrollbars=yes");
			}
		});
		...
		$("#kodebarang").keyup(function(e) {
			...
			$.ajax({
				url: 'inc/cari_barang_kanvas.php',
				type: 'post',
		...
		$("#hapussemuaitem").click(function() {
			$('#hasil').load("inc/delete_tmp_kanvas.php");
			$("#totalharga").val("");
			$("#batalitem").click();
		});
...
<?php
	...
	include "data_kanvas.php";
} else if ($_GET['action'] == 'edit') {
	include "edit_kanvas.php";
} else if ($_GET['action'] == "delete") {
	include "delete_kanvas.php";
} else if ($_GET['action'] == "delete_detail") {
	include "delete_detail_kanvas.php";
}
?>