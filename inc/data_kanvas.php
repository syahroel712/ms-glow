		...
		<div class="row mb-2">
			...
			<div class="col-sm-6">
				<ol class="breadcrumb float-sm-right">
					...
					<li class="breadcrumb-item active">Data Kanvas</li>
				</ol>
			</div><!-- /.col -->
		</div><!-- /.row -->
	</div><!-- /.container-fluid -->
			...
			<div class="col-lg-12">
				<div class="card">
					<div class="card-header">
						<h3 class="card-title">
							Data Kanvas (<b> <?= tgl_indo($tanggal) ?> <?php if ($_GET['no'] != "") {
																															echo "Nota : $_GET[no]";
																														} ?> </b>)
						</h3>
					</div>
						...
						?>
							<div class="table-responsive-sm">
								<table id="example1" class="table table-bordered">
									...
									<tbody id="pembelian">
										<?php
										$sql = mysql_query("select * from tb_barang_kanvas where nota='$_GET[no]'") or die(mysql_error());
							...
							<div class="container mt-4">
								<div class="table-responsive-sm">
									<table id="example1" class="table table-bordered">
										...
										<tbody id="pembelian">
											<?php
											$sql = mysql_query("SELECT * FROM tb_kanvas $squ ORDER BY tgl_beli DESC");
											$cek = mysql_num_rows($sql);
											if ($cek < 1) {
											?>
												<tr>
												...
												$no = 1;
												while ($data = mysql_fetch_array($sql)) {
													$sup = mysql_fetch_array(mysql_query("SELECT * FROM tb_member WHERE id_member='$data[id_member]'"));

													$tot = mysql_fetch_array(mysql_query("SELECT *,SUM(harga_akhir) AS tot,(SUM(harga_akhir)-(SUM(harga_akhir) * ('$data[diskon_persen] . '/100))) AS tot2 FROM tb_barang_kanvas WHERE nota='$data[nota]' group by nota"));
												?>
													<tr>
														<td>
															<a href='?page=kanvas&action=view&no=<?php echo $data['nota']; ?>'><?= $data['nota']; ?></a>
														</td>
														...
														<?php if ($_SESSION['admin']) { ?>
															<td>
																<a href="cetak_struk_kanvas.php?id=<?php echo $data['nota']; ?>" target="_blank">
																	<button class="btn btn-success btn-xs shadow" title="Print Data"><span class='fa fa-print'></span></button>
																</a>
																<a onclick="return confirm('Yakin ingin menghapus data ?');" href="?page=kanvas&action=delete&id=<?php echo $data['nota']; ?>">
...
<script type="text/javascript">
	$("#caripembelian").click(function() {
		...
		$.ajax({
			url: 'inc/proses_cari_kanvas.php',
			type: 'post',
	...
	$("#pembelian").on("click", "#lihatbarang", function() {
		...
		$.ajax({
			data: 'no=' + no,
			url: 'inc/lihat_barang_kanvas.php',
			type: 'post',
			success: function(msg) {
